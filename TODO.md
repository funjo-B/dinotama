...
- [x] 스프라이트 128x128로 확대 (72→128)
- [x] 종별 고유 실루엣 — 각 공룡 특징 반영 (랩터 낫발톱, 티렉스 거대 머리, 프테라노돈 날개 등)
- [x] 스테이지별 크기 차이 (baby 45% → teen 70% → adult 100%)
- [x] 감정별 눈 표현 (happy ^^, sad 눈물, sleepy --, hungry 큰 눈)
- [x] 알은 종 고유 색상 점무늬 + 금 + 그림자
- [x] 가챠 패널 (별도 창) — 확률 정보, 종별 목록, 천장 카운터, 뽑기 버튼
- [x] 가챠 모션 클릭 제어 — 클릭하면 바로 결과 공개
- [x] 합성 모션 — before(3마리 모임) → flash(번쩍) → after(진화 결과)
- [x] 버튼 즉시 툴팁 (마우스 올리면 설명)
